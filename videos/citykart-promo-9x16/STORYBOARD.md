...
- **縦型 (1080x1920) の決まり** — 走行映像は 16:9 の素材を object-fit: cover で敷くので、左右が切れて
  中央だけが見える。看板ゲートや橋の主塔は画面中央にあるため問題ない。文字は横幅が半分になるので、
  街名は2行に折らず入る大きさまで落とし、距離は改行して下に置く。文字の塊は画面の下から 1/3 に置き、
  上 2/3 は映像を見せる（縦では上下に余白が生まれるため、横位置ではなく高さで構図を作る）。
