...
description: >
  ...
  Hiragino Sans in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
# 自動変換 (build-frame.mjs) は地と文字を取り違えて白地×濃紺文字にしていたので、
# citykart.jp の実物に合わせて手で直した。LP は濃紺 (#0B1A2A) の地に淡色の文字、
# 黄色 (#FFD83D) が唯一の差し色。走行映像も暗いので、地が白いと画が衝突する。
# 値は LP の CSS 変数そのまま: --bg / --bg-2 / --fg / --muted / --accent / --line。
colors:
  ink-black: "#0B1A2A"
  ink-black-alt: "#0F2135"
  fire-orange: "#FFD83D"
  cream: "#EEF1F4"
  cream-muted: "#9AA8B6"
  cream-hint: "#7F8C99"
...
typography:
  # — reading ramp —
  body:    { fontFamily: "Hiragino Sans", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "Hiragino Sans", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "Hiragino Sans", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "IBM Plex Mono", cqw: 0.72, weight: 400, tracking: "0.14em", upper: true }
  # — display / hero ramp (Hiragino Sans, lowercase, negative tracking) —
  h3:      { fontFamily: "Hiragino Sans", cqw: 2.8, weight: 700, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "Hiragino Sans", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "Hiragino Sans", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "Hiragino Sans", cqw: 5.5, weight: 900, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Hiragino Sans", cqw: 7.5, weight: 900, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Hiragino Sans", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Hiragino Sans", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Hiragino Sans", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Hiragino Sans", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **Hiragino Sans** (display) / **Hiragino Sans** (body); ignore any preset font name lingering in prose.
...
text and becomes graphic primitive.** Hiragino Sans `display` at 13cqw puts a single lowercase word
...
**Hiragino Sans** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Hiragino Sans 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Hiragino Sans body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Hiragino Sans `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Hiragino Sans display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Hiragino Sans `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Hiragino Sans lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Hiragino Sans `display`/`h1`
...
row of three top-border-only stat-cards — big Hiragino Sans-900 numeral in `{colors.fire-orange}`, Hiragino Sans
...
**Ground** ink-black. **Composes** fadelist (3 stacked Hiragino Sans-900 words at 1.0/0.5/0.22), fadelist-title.
...
Hiragino Sans `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Hiragino Sans display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Hiragino Sans display; never add a second accent color.
...
- No serif companion; chrome is never Hiragino Sans.
...
- **Type** — Hiragino Sans lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Hiragino Sans + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
